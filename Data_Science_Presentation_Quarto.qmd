---
title: "Analysis for 'Neighborhoods Matter: Assessing the Evidence for Place Effects'"
author: "Mahsa Abdollahi Mirzanagh"
date: "2024-06-13"
format: 
  html:
...
I use data on various outcomes such as employment rate, life expectancy, and upward mobility to assess the impact of poverty rates on these outcomes.

# 1. Loading the initial libraries and data
...
#### 2.1.1 Panel A. Adult employment rate 2000
...
ggplot(cz_adult_emp_pov, aes(x = pov_rate, y = emp2000)) +
  ...
  labs(x = "Poverty rate percentage point Mean", y = "Adult employment rate 2000 Mean")
...
ggplot(new_cz_life_expect_pov, aes(x = pov_ratemean, y = le_aggmean)) + 
  geom_point()+
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Poverty Rate (percentage point) Mean", y = "Life Expectancy Mean (CZ level)") +
  theme_minimal()
...
#### 2.1.3.Panel C. Intergenerational mobility for low-income children
...
ggplot(new_cz_upward_mobility_pov, aes(x = pov_ratemean, y = kfr_pooled_p25_czmean)) + 
  geom_point()+
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Poverty Rate (percentage point) Mean", y = "Upward mobility, p25 (CZ-level)") +
  theme_minimal()


```

#### 2.1.4. Panel D. Academic achievement

```{r}
# Panel D. Academic achievemen
...
ggplot(new_district_test_scores_pov, aes(x = pov_ratemean, y = mn_avg_olmean)) + 
  geom_point()+
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Poverty Rate (percentage point) Mean", y = "Mean Test Score (SD level) Mean") +
  theme_minimal()
...
ggplot(new_tract_adult_emp_pov, aes(x = pov_ratemean, y = emp2000mean)) + 
  geom_point()+
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Poverty Rate Mean", y = "Tract Adult Employment") +
  theme_minimal()


```

#### 2.2.2. Tract Upward Mobility
...
new_tract_upward_mobility_pov = tract_upward_mobility_pov %>% group_by(bin) %>% summarise(kfr_pooled_p25mean = mean(kfr_pooled_p25, na.rm = TRUE), pov_ratemean = mean(pov_rate)) #find the x and y mean of each bin

ggplot(new_tract_upward_mobility_pov, aes(x = pov_ratemean, y = kfr_pooled_p25mean)) + 
  geom_point()+
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Poverty Rate Mean", y = "Track Upward Mobility") +
  theme_minimal()


```

#### 2.2.3. District Test Score
